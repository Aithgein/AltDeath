...
{
    bool expected = false;
    if (!_checkpointPending.compare_exchange_strong(expected, true)) {
        return;
    }

    const auto taskInterface = SKSE::GetTaskInterface();
    if (!taskInterface) {
        _checkpointPending.store(false);
        spdlog::error("SKSE task interface was unavailable for checkpoint capture");
        Notify("Shades Respawn: checkpoint failed");
        return;
    }

    // Sleep/wait stop events fire while the menu and player state are still
    // being finalized. Creating or moving a reference from inside that event
    // can fail. Run the actual checkpoint operation on the next game update.
...
void CheckpointManager::CaptureCheckpointNow()
{
    const auto player = RE::PlayerCharacter::GetSingleton();
    if (!player) {
        spdlog::error("Could not capture checkpoint: player unavailable");
        Notify("Shades Respawn: checkpoint failed");
        return;
    }
    ...
    if (!_xMarkerBase) {
        spdlog::error("Could not capture checkpoint: XMarker base unavailable");
        Notify("Shades Respawn: checkpoint failed");
        return;
    }
    ...
    if (!marker) {
        spdlog::error("Failed to create the runtime checkpoint marker on deferred game task");
        Notify("Shades Respawn: checkpoint failed");
        return;
    }
