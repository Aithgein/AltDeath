...
void CheckpointManager::InitializeForms()
{
    ...
    if (!dataHandler) {
        SKSE::log::error("TESDataHandler was unavailable");
        return;
    }

    _xMarkerBase = dataHandler->LookupForm<RE::TESBoundObject>(kXMarkerLocalID, kSkyrimPlugin);
    if (!_xMarkerBase) {
        SKSE::log::error("Could not resolve XMarker {:08X} from {}", kXMarkerLocalID, kSkyrimPlugin);
    }
...
void CheckpointManager::CaptureCheckpoint()
{
    ...
    if (auto marker = ResolveMarker()) {
        ...
        SKSE::log::info("Updated respawn checkpoint at marker {:08X}", marker->GetFormID());
        return;
    }

    const auto marker = player->PlaceObjectAtMe(_xMarkerBase, true);
    if (!marker) {
        SKSE::log::error("Failed to create the runtime checkpoint marker");
        return;
    }
    ...
    SKSE::log::info("Created respawn checkpoint marker {:08X}", marker->GetFormID());
}

void CheckpointManager::QueueTeleport()
{
    if (!ResolveMarker()) {
        SKSE::log::warn("Shades resurrected the player, but no sleep/wait checkpoint exists yet");
        return;
    }
    ...
    if (!taskInterface) {
        ...
        SKSE::log::error("SKSE task interface was unavailable");
        return;
    }
...
void CheckpointManager::TeleportNow()
{
    ...
    if (!player || !marker) {
        SKSE::log::error("Checkpoint vanished before teleport could run");
        return;
    }

    player->MoveTo(marker);
    SKSE::log::info("Teleported player to checkpoint {:08X}", marker->GetFormID());
}
...
void CheckpointManager::Save(SKSE::SerializationInterface* a_intfc)
{
    ...
    if (!a_intfc->WriteRecordData(&markerID, sizeof(markerID))) {
        SKSE::log::error("Failed to write checkpoint marker FormID");
    }
}
...
void CheckpointManager::Load(SKSE::SerializationInterface* a_intfc)
{
    ...
    while (a_intfc->GetNextRecordInfo(type, version, length)) {
        if (type != kRecordType) {
            SKSE::log::warn("Skipping unknown serialization record {:08X}", type);
            continue;
        }
        ...
        if (a_intfc->ReadRecordData(&savedID, sizeof(savedID)) != sizeof(savedID)) {
            SKSE::log::error("Failed to read checkpoint marker FormID");
            continue;
        }

        RE::FormID resolvedID = 0;
        if (savedID != 0 && !a_intfc->ResolveFormID(savedID, resolvedID)) {
            SKSE::log::warn("Could not resolve saved checkpoint marker {:08X}", savedID);
            resolvedID = 0;
        }
        ...
        SKSE::log::info("Loaded checkpoint marker {:08X}", resolvedID);
    }
}
